...
router.get("/fetchEnded/:user", (req, res)=>{
    const user = req.params.user;
    const query = "SELECT * FROM request INNER JOIN users on (request.user = users.email || request.receiver = users.email) WHERE (request.user = ? or request.receiver = ?) and request.status = 'Ended' ";
